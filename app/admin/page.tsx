...
function SongCard({ song, label }: { song: Song; label?: string }) {
  ...
  return (
    <div className="flex flex-col gap-2 border border-[color:var(--color-border)] rounded-xl p-4">
      ...
      <p className="text-lg font-bold leading-snug">{stripFeaturing(song.synonymTitle)}</p>
...
export default function AdminPage() {
  ...
  const unscheduledByGenre = Array.from(new Set(unscheduled.map((s: Song) => s.genre ?? "Other")))
    .sort()
    .map((genre) => ({
      genre,
    ...
    <main className="flex flex-col items-center px-4 pt-8 pb-16">
      <div className="w-full max-w-[700px] flex flex-col gap-8">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "Total Songs", value: library.length },
            { label: "Days Scheduled", value: scheduledDays.length },
            { label: "Unscheduled", value: unscheduled.length },
          ].map(({ label, value }) => (
            <div
              key={label}
              className="flex flex-col gap-1 bg-[color:var(--color-card)] border border-[color:var(--color-border)] rounded-xl p-4"
            >
        ...
        <div className="flex flex-col gap-3">
          <h2 className="text-sm font-bold uppercase tracking-widest text-[color:var(--color-muted)]">Library by Genre</h2>
        ...
        {unscheduled.length > 0 && (
          <>
            ...
            {unscheduledByGenre.map(({ genre, songs }) => (
